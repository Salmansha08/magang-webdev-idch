...
;(function (global, factory) {
  ...
  function configFromArray (config) {
    let i; let date; const input = []; let currentDate; let yearToUse
  ...
  const aspNetRegex = /^(\-)?(?:(\d*)[. ])?(\d+)\:(\d+)(?:\:(\d+)\.?(\d{3})?\d*)?$/
  ...
  function moment_calendar__calendar (time, formats) {
    ...
    const now = time || local__createLocal()
    const sod = cloneWithOffset(now, this).startOf('day')
    const diff = this.diff(sod, 'days', true)
    const format = diff < -6
      ? 'sameElse'
      : diff < -1
        ? 'lastWeek'
    ...
    const output = formats && (isFunction(formats[format]) ? formats[format]() : formats[format])
  ...
  locale_locales__getSetGlobalLocale('en', {
    ...
    ordinal: function (number) {
      const b = number % 10
      const output = (toInt(number % 100 / 10) === 1)
        ? 'th'
        : (b === 1)
            ? 'st'
            : (b === 2)
